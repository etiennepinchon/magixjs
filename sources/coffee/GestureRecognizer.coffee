...
class GestureRecognizer
	...
	touchstart: (event) =>
		...
		@session =
			...
			startTime: Date.now()
			pressTimer: window.setTimeout(@longpressstart, GestureLongPressTime * 1000)
			started: {}
			events: []
			eventCount: 0
		...
		if Date.now() - @doubleTapTime < (GestureDoubleTapTime * 1000)
			@doubletap(event)
		else
	...
	_updateTouchForce: =>
		...
		if @session.force >= GestureForceTapMobile
			@forcetapstart(event)
		else
			@forcetapend(event)

		setTimeout(@_updateTouchForce, GestureForceTapMobilePollTime)
	...
	swipedirectionstart: (event) =>
		...
		if swipeEdge is "top" and 0 < event.start.y - App.frame.y < GestureEdgeSwipeDistance
			@edgeswipedirectionstart(event)
		if swipeEdge is "right" and maxX - GestureEdgeSwipeDistance < event.start.x < maxX
			@edgeswipedirectionstart(event)
		if swipeEdge is "bottom" and maxY - GestureEdgeSwipeDistance < event.start.y < maxY
			@edgeswipedirectionstart(event)
		if swipeEdge is "left" and 0 < event.start.x - App.frame.x < GestureEdgeSwipeDistance
			@edgeswipedirectionstart(event)
	...
	edgeswipedirection: (event) =>
		...
		# Direction of the gesture: "left"
		swipeEdge = @_edgeForSwipeDirection(@session.started.edgeswipedirection.offsetDirection)
		
		App.emit("edgeswipe", @_createEvent("edgeswipe", event))
		App.emit("edgeswipe#{swipeEdge}", @_createEvent("edgeswipe#{swipeEdge}", event))

		@_dispatchEvent("edgeswipe#{swipeEdge}", event)

	edgeswipedirectionstart: (event) =>
		return if @session.started.edgeswipedirection
		@session.started.edgeswipedirection = event
		swipeEdge = @_edgeForSwipeDirection(@session.started.edgeswipedirection.offsetDirection)
		App.emit("edgeswipestart", @_createEvent("edgeswipestart", event))
		App.emit("edgeswipe#{swipeEdge}start", @_createEvent("edgeswipe#{swipeEdge}start", event))

	edgeswipedirectionend: (event) =>
		swipeEdge = @_edgeForSwipeDirection(@session.started.edgeswipedirection.offsetDirection)
		App.emit("edgeswipeend", @_createEvent("edgeswipeend", event))
		App.emit("edgeswipe#{swipeEdge}end", @_createEvent("edgeswipe#{swipeEdge}end", event))
	...
	_process: (event) =>
		...
		if not @session.started.swipe and (
			Math.abs(event.offset.x) > GestureSwipeThreshold or
			Math.abs(event.offset.y) > GestureSwipeThreshold)
				@swipestart(event)
