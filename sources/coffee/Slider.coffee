...
class Slider extends View
	...
	_touchStart: (event) =>
		...
		if @width > @height
			@value = @valueForPoint(Event.touchEvent(event).viewX)#@valueForPoint(Event.touchEvent(event).clientX - @x - offsetX) #- @screenScaledFrame().x) / @canvasScaleX() - offsetX
		else
			@value = @valueForPoint(Event.touchEvent(event).viewY)#@valueForPoint(Event.touchEvent(event).clientY - @y - offsetY)# - @screenScaledFrame().y) / @canvasScaleY() - offsetY
