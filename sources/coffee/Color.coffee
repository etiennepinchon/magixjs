...
class Color extends Element
	...
	toName: ->
		...
		for key in Utils.keys cssNames
			value = cssNames[key]
			if value == hex
				return key
	...
	@toColor: (color) ->
		...
		# Gradient
		if color.color and Utils.isString(color.color) and color.color.indexOf('gradient') isnt -1
			return color.color
...
ColorModel.isRGB = (colorModel) ->
	return colorModel.toLowerCase() in [ColorModel.RGB, ColorModel.RGBA] if Utils.isString(colorModel)
	return false

ColorModel.isHSL = (colorModel) ->
	return colorModel.toLowerCase() in [ColorModel.HSL, ColorModel.HSLA] if Utils.isString(colorModel)
	return false
	...
	if color == null
		...
		if typeof color == "object"
			...
			if color.hasOwnProperty("r") or color.hasOwnProperty("g") or color.hasOwnProperty("b")
				...
				if Utils.isString(color.s) then s = numberFromString(color.s)
				l = if isNumeric(color.l) then color.l else 0.5
				if Utils.isString(color.l) then l = numberFromString(color.l)
