...
class BezierCurveAnimator extends Animator
  ...
  setup: (options) ->
    ...
    if Utils.isString(options) and BezierCurveDefaults.hasOwnProperty options.toLowerCase()
      options = { values: BezierCurveDefaults[options.toLowerCase()] }

    # Input values is one of the named bezier curves
    if options.values and Utils.isString(options.values) and BezierCurveDefaults.hasOwnProperty options.values.toLowerCase()
      options = { values: BezierCurveDefaults[options.values.toLowerCase()], time: options.time }

    # Input is a single array of 4 values
    if Utils.isArray(options) and options.length is 4
